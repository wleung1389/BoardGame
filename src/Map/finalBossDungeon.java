	...
	public void enterRoom(Player x) {
		...
		while(monster1.getHP() != 0 && monster2.getHP() != 0 && x.getChp() > 0)
		{
			...
			if(choice.substring(0,1).equalsIgnoreCase("F"))
			{
				...
				else
				{
					...
					System.out.println("You attacked the first monster with your weapon. You dealt " + dmg + ". The monster has " + monster1.getHP() + " HP left.");
		...
		while(boss.getHP() > 0)
		{
			...
			if(choice.substring(0,1).equalsIgnoreCase("F"))
			{
				...
				double dmg = (wepAttk * Runner.getRndInteger(6,10))*((rnd/10) * monster1.getDefense());
				boss.attacked(dmg);
				System.out.println("You attacked the boss with your weapon. You dealt " + dmg + ". The boss has " + boss.getHP() + " left.");
